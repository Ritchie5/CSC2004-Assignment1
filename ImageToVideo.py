...
# to get the paylaod's size, data
def get_payload(file, itype):
    try:
...
def Encode(payload, coverobj, bits):
    ...
    payload_details = get_payload(payload)
    payload_base64 = to_base64(payload)
    binary_payload_data = to_binary(payload_base64)
    payload_len = len(binary_payload_data)

    # Convert cover object into binary
    coveronj_details = get_object(coverobj)
    coverobj_base64 = to_base64(coverobj)
    binary_coverobj_data = to_binary(coverobj_base64)
    coverobj_len = len(binary_coverobj_data)
...
def main():
    ...
    try:
        ...
        if start_step == 1:
            ...
            payload = r'C:\Users\ivanc\Cyber Security Fundamentals\CSC2004-Assignment1\Image\image1.png' #input("Input payload to Hide (inc. extension): ")
