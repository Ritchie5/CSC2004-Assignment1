...
def encode():
    ...
    frame_bytes = bytearray(list(audio.readframes(
        audio.getnframes())))  # convert to byte data
    # print("OG:")
    # for i in range(0,8):
    #    print(format(frame_bytes[i],'08b'))
    # print(frame_bytes)
    #string = "WOOOOOOOOOOOOOOOOOO"
    string = open("Audio\\okay-1.wav","rb")
    f=string.read()
    delimiter="#####"
    delimiter=bytearray(delimiter,'utf-8')
    print(delimiter)
    ...
    for i in range(len(frame_bytes)):
        if counter < len(binarystring):
            # print(frame_bytes[i])
            frame_bytes[i] = int(str(format(frame_bytes[i], '08b')[
                                 :-1])+binarystring[counter], 2)
            # print(frame_bytes[i])
            counter += 1
        if counter >= len(binarystring):
            break
