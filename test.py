    ...
    for i in range(len(frame_bytes)):
        ...
        for x in range(len(bitPos)):
            if counter < len(binarystring):
                ...
                listBytes[bitPos[x]]=binarystring[counter]
...
def decode(bitPos=[5,7]):
    ...
    for i in range(len(frame_bytes)):
        ...
        #string+=(format(frame_bytes[i],'08b')[-1])
          string+=(format(frame_bytes[i],'08b')[bitPos[x]])
    #print(string)
    binaryPayloadFormat = [string[i:i+8]for i in range(0, len(string), 8)]
    payload = ""
